...
1. §6's claim that cloud transcription makes pre-roll impossible is **confirmed and quantified.**
...
### a. Signing is a ship gate for the revamp — but not a build gate for any phase
...
| ---------------------------------- | ---------------------------------------------------------------------- |
| Always-open microphone (§3)        | `NSMicrophoneUsageDescription` + user consent. Works ad-hoc.           |
| Accessibility text injection (§7)  | User grants it in System Settings. No entitlement, no certificate.     |
| ScreenCaptureKit system audio (§8) | Screen Recording TCC grant. No entitlement, no certificate.            |
| Global hotkey                      | `RegisterEventHotKey` needs nothing; `CGEventTap` needs Accessibility. |

The entitlements in `build/entitlements.mac.plist` only matter under Hardened Runtime, which is only\
 required for notarization, which is only required for distribution. Evidence on this machine: the\
 installed `/Applications/Noteato.app` is `adhoc, linker-signed` with `TeamIdentifier=not set` — and\
 it runs today with microphone access. Contributors building from source need nothing: locally built\
 apps never receive the `com.apple.quarantine` xattr, and arm64's mandatory ad-hoc signature is\
 applied automatically by the linker.
 ...
 ** Noteato update re-prompts every user for Accessibility and Screen Recording.** Tolerable for today's\
 ...
 central claim is that audio never leaves the machine.

**So: enrollment gates the release of Phases 1–4, not the development of any phase.** It is a\
 parallel track with no code dependency, and it should start early only because it has a lead time\
 the code doesn't. Phases 2–6 can be built and tested end-to-end unsigned.

One item I would rather verify than assert: macOS 15 introduced periodic re-authorization prompts\
 for screen capture. I do not know the current 26.x behaviour, or whether signing affects it. Worth\
 an empirical check during Phase 6 rather than planning around recollection.
...
development of any phase. Parallel track, no code dependency, but start it early for the lead time.

Everything else in the brief I can proceed on as written.

The build plan is in `phase-plan.md`.
