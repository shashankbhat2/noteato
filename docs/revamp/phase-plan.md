...
**1. Note surfaces: Note / Transcript tabs plus a per-note Chat drawer.** The two old chat surfaces
(`AgentPanel` in a pane, and the second assistant in `HomeView`) remain gone. Each note owns two
document surfaces in a compact tab row below its title and before its metadata:

- **Note** → the block editor.
- **Transcript** → visible but disabled until that note has recording metadata from Phase 2 and a
  transcript from Phase 3. It is UI scaffolding, not a second capture path.

**Chat** is no longer a third page surface. A gesture-style pill at the bottom of the note opens a
glass drawer over the document. The note-level conversation and whole-note actions stay scoped to
that note, and the drawer remains mounted while collapsed so its draft and history survive.
...
*Note on §11:* the brief rules out a chat surface outright. The drawer is still one, scoped and
threaded per note. Built as decided; flagged here so it stays a decision rather than becoming drift.
...
### B. The AI surface · M — ✅ done (now the per-note Chat drawer — see STATUS.md)

- `NoteAiPanel`, mounted inside each `NoteEditor`: a bottom glass drawer holds Chat + whole-note
  actions without replacing the writing surface.
- The Transcript tab is disabled until Phase 2/3 provide recording and transcript state.
...
**Proof** — Note and Transcript remain document surfaces, Chat opens from a collapsed gesture pill,
the title has no bubble menu, and Transcript cannot imply unavailable recording infrastructure.
